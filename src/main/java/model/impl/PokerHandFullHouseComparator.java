...
public class PokerHandFullHouseComparator implements PokerHandComparator {
    @Override
    public PokerHandComparatorResult compare(Hand firstHand, Hand secondHand) {
        ...
        Optional<Map.Entry<HandValue, Long>> first = firstHandValuesByCount.entrySet().stream()
                .filter(handValueLongEntry -> handValueLongEntry.getValue() == 3)
                .findFirst();
        ...
        if (first.isPresent() && second.isPresent()) {
            ...
            if (firstHandValue.rank < secondhandValue.rank) {
                return new PokerHandComparatorResult(firstHand, Collections.singletonList(secondhandValue));
            }
        }
